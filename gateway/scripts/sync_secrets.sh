...
bedrock_bearer_token="$(op_read_ref "${GATEWAY_BEDROCK_BEARER_TOKEN_REF}")" || die "Unable to read Bedrock bearer token from 1Password."
...
[[ -n "${bedrock_bearer_token}" ]] || die "Bedrock bearer token was empty."
...
printf '%s' "${bedrock_bearer_token}" | npx wrangler secret put AWS_BEARER_TOKEN_BEDROCK --cwd "${GATEWAY_RENDER_DIR}" >/dev/null
